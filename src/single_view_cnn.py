...
MODEL_DIR = os.path.join(args.out, f"{args.architecture}-{TIMESTAMP}-{args.name}")
...
CALLBACKS = [
    ...
    tf.keras.callbacks.ReduceLROnPlateau(
        monitor="val_loss", factor=0.5, patience=3, verbose=1, mode="min", min_lr=1e-8
    ),
...
def generate_cnn(app="vgg"):
    ...
    if app == "vgg":
        ...
        net = keras.applications.EfficientNetB0(include_top=False, weights="imagenet")
        # net.trainable = False
        ...
        net = keras.applications.Xception(
            include_top=False,
            weights="imagenet",
        )
        net.trainable = False
        ...
        x = keras.layers.Conv2D(
            96, kernel_size=7, strides=2, padding="same", kernel_regularizer="l2"
        )(inputs)
        ...
        x = keras.layers.Conv2D(
            256, kernel_size=5, strides=2, padding="same", kernel_regularizer="l2"
        )(x)
        x = layers.LeakyReLU()(x)
        x = keras.layers.BatchNormalization()(x)
        x = keras.layers.MaxPool2D(pool_size=3, strides=2)(x)
        x = keras.layers.Dropout(0.5)(x)
        x = keras.layers.Conv2D(
            512, kernel_size=3, strides=1, padding="same", kernel_regularizer="l2"
        )(x)
        x = layers.LeakyReLU()(x)
        x = keras.layers.Dropout(0.5)(x)
        x = keras.layers.Conv2D(
    ...
    model.summary()
    losses = {"class": "categorical_crossentropy", "view": "categorical_crossentropy"}
    model.compile(
        keras.optimizers.Adam(learning_rate=float(args.lr)),
        loss=losses,
        metrics=METRICS,
    )
...
def main():
    ...
    if args.getpred is None:
        ...
        hist_df.to_csv(
            os.path.join(
                MODEL_DIR,
                f"{TIMESTAMP}_{args.train_sample_ratio}_{args.test_sample_ratio}_training_history.csv",
            )
        )
    else:
