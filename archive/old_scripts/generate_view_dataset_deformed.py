...
def nonblocking_custom_capture(tr_mesh, rot_xyz, last_rot):
    ...
    os.makedirs(os.path.join(OUT_DIR, ViewData.mname,"image", str(ViewData.sigma), ), exist_ok=True)
...
for label in [x for x in os.listdir(DATA_PATH) if Path.is_dir(Path(DATA_PATH, x))]:
    ...
    file_list.extend(
        [os.path.join(DATA_PATH, label, args.set, x) for x in os.listdir(
            os.path.join(DATA_PATH, label, args.set))][:50]
    )
